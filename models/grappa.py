...
class GrappaModel(torch.nn.Module):
    def __init__(
        ...
        in_feats: int = None,
        in_feat_name: Union[str, List[str]] = [
            "atomic_number",
            "ring_encoding",
            "partial_charge",
            "degree",
        ...
        gnn_dropout_initial: float = 0.0,
        gnn_dropout_conv: float = 0.0,
        ...
        self.parameter_writer = WriteParameters(
            ...
            nonbonded_hidden_feats=nonbonded_hidden_feats,
        )
        ...
        for lvl in ["n2", "n3", "n4", "n4_improper"]:
            if lvl in g.ntypes:
                if len(g.nodes[lvl].data["idxs"]) > 0:
                    ...
                    max_idx = 0
                assert (
                    max_idx <= g.num_nodes("n1")
                ), f'Encountered idxs up to {max_idx} at the level g.nodes[{lvl}].data["idxs"], but there are only {g.num_nodes("n1")} atom-level-nodes in the graph'
