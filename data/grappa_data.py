...
class GrappaData(pl.LightningDataModule):
    def __init__(
        ...
        self.ref_terms = ref_terms if ref_terms is not None else ["nonbonded"]
        """
    ...
    def setup(self, stage: str = None):
        ...
        GrappaData._check_ds_disjoint(
            self.datasets,
            self.pure_train_datasets,
            self.pure_val_datasets,
            self.pure_test_datasets,
        )
        ...
        if len(self.datasets) > 0:
            ...
            dataset += [
                Dataset.from_tag(p)
                for p in tqdm(self.datasets, desc="Loading datasets")
            ]

        pure_train_sets = [
            ds for ds in self.pure_train_datasets if isinstance(ds, Dataset)
        ]
        if len(pure_train_sets) > 0 or len(self.pure_train_datasets) > 0:
            logging.info(
                "Loading pure train datasets from:\n"
                + "\n".join([str(p) for p in self.pure_train_datasets])
            )
            pure_train_sets += [
                Dataset.from_tag(p)
                for p in tqdm(self.pure_train_datasets, desc="Loading pure train datasets")
            ]
        ...
        if len(pure_val_sets) > 0 or len(self.pure_val_datasets) > 0:
            logging.info(
                "Loading pure val datasets from:\n"
                + "\n".join([str(p) for p in self.pure_val_datasets])
            )
            pure_val_sets += [
                Dataset.from_tag(p)
                for p in tqdm(self.pure_val_datasets, desc="Loading pure val datasets")
            ]

        pure_test_sets = [
            ds for ds in self.pure_test_datasets if isinstance(ds, Dataset)
        ]
        if len(pure_test_sets) > 0 or len(self.pure_test_datasets) > 0:
            logging.info(
                "Loading pure test datasets from:\n"
                + "\n".join([str(p) for p in self.pure_test_datasets])
            )
            pure_test_sets += [
                Dataset.from_tag(p)
                for p in tqdm(self.pure_test_datasets, desc="Loading pure test datasets")
            ]
        ...
        if self.split_ids is not None:
            assert isinstance(
                self.split_ids, dict
            ), f"split_ids must be a dictionary, but got {type(self.split_ids)}"
            assert self.split_ids.keys() == {
                "train",
                "val",
                "test",
            }, f"split_ids must have keys 'train', 'val', and 'test', but got {self.split_ids.keys()}"
        ...
        self.split_ids = dataset.calc_split_ids(
            partition=self.partition, seed=self.seed, existing_split=self.split_ids
        )
        ...
        if self.save_splits is not None:
            ...
            ), f"save_splits must be a Path or str, but got {type(self.save_splits)}"
        ...
        if self.max_energy is not None:
            logging.info(
                f"Filtering out conformations with energy difference > {self.max_energy} to the minimal energy."
            )
            logging.info(
                f"Filtering out conformations with force norm > {self.max_force}."
            )
        ...
        if self.tr_subsampling_factor is not None:
            if self.tr_subsampling_factor == 0.0:
                logging.warning(
                    "Subsampling factor is 0, training set will be empty."
                )
        ...
        if self.tr_max_confs is not None:
            if self.tr_max_confs == 0:
                logging.warning(
                    "Maximum number of conformations is 0, training set will be empty."
                )
        ...
        # Для подхода B: energy_ref = energy_qm, никаких FF
        self.tr.create_reference(ref_terms=[], ff_lookup={}, cleanup=False)
        self.vl.create_reference(ref_terms=[], ff_lookup={}, cleanup=False)
        self.te.create_reference(ref_terms=[], ff_lookup={}, cleanup=False)
        ...
        for i in range(3):
            for j in range(i):
                ...
                if len(set(ds_paths_1).intersection(set(ds_paths_2))) > 0:
                    print(
                        f"Warning: Dataset paths {ds_paths_1} and {ds_paths_2} are not disjoint: {set(ds_paths_1).intersection(set(ds_paths_2))}"
                    )
    ...
    def train_dataloader(self):
        return GraphDataLoader(
            ...
            conf_strategy=self.conf_strategy,
            weights=self.weights,
            balance_factor=self.balance_factor,
            drop_last=len(self.tr) > self.train_batch_size,
        )
    ...
    def val_dataloader(self):
        return GraphDataLoader(
            ...
            pin_memory=self.pin_memory,
            conf_strategy=self.val_conf_strategy,
            drop_last=False,
        )
