...
class MolwiseLoss(torch.nn.Module):
    ...
    def forward(self, g, dsnames:List[str]=None):
        ...
        for i, graph in enumerate(graphs):
            ...
            # В MolwiseLoss.forward, после loss_term:
                if 'sigma' in graph.nodes['n1'].data:
                    sigma = graph.nodes['n1'].data['sigma']
                    epsilon = graph.nodes['n1'].data['epsilon']
    # Штраф за отклонение от разумных значений:
                    loss_term += 1e-3 * torch.mean((sigma - 3.5)**2)
                    loss_term += 1e-2 * torch.mean((epsilon - 0.1)**2)
            if 'alpha_coul' in graph.nodes['n1'].data:
                alpha_coul = graph.nodes['n1'].data['alpha_coul']
                r0_coul = graph.nodes['n1'].data['r0_coul']
                loss_term = loss_term + 1e-3 * torch.mean((alpha_coul - 20.0)**2)
                loss_term = loss_term + 1e-2 * torch.mean((r0_coul - 0.3)**2)
